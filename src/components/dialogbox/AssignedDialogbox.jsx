...
export default function AssignLeadDialog({
  ...
  if (broadcastAll) {
    // ✅ Broadcast to all
    onSubmit({ assigned_to: "all" });
  } else if (selectedAgent) {
    // ✅ Single agent
    onSubmit({ assigned_to: selectedAgent });
  }
  onClose();
};
    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        >
          ...
          >
            Assign this lead to one or all agents
          </Dialog.Description>
          ...
          <div className="mt-6 flex justify-end gap-3">
            ...
            <button
              onClick={handleAssign}
              disabled={!broadcastAll && !selectedAgent}
              className="rounded-md bg-blue-600 px-4 py-2 text-sm text-white cursor-pointer
                hover:bg-blue-700 
                disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {broadcastAll ? "Broadcast" : "Assign"}
            </button>
          </div>
