...
const FormDialog = ({
  ...
  // Build Yup schema dynamically based on required fields
  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger asChild>
        <button
          type="button"
          className="inline-flex items-center gap-1 px-4 py-2 text-sm rounded bg-green-500 text-white hover:bg-green-600"
        >
          + {triggerLabel}
        </button>
      </Dialog.Trigger>
  ...
  `}
          ...
          <Dialog.Close asChild>
            <button
             onClick={() => formik.resetForm()}  
              className={`absolute top-4 right-4 ${isDark ? "text-white" : "text-gray-500"
                } cursor-pointer`}
            >
              <Cross2Icon />
            </button>
